...
const loadSchema = () => {
  ...
  const schemaMatch = content.match(
    /const schema\s*=\s*({[^]*});\s*export default/,
  );
  if (!schemaMatch) {
...
const validateEnv = () => {
  ...
  const missingInExample = envVariables.filter(
    (varName) => !envExampleVariables.includes(varName),
  );
  const missingInValidation = envVariables.filter(
    (varName) => !validatedVariables.includes(varName),
  );
